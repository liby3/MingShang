...
public class NoCharacter extends Character {
	...
	public int judgeAction(int k, int m, int i, int j, int[][] theTeam, int[][] numberOfRoles, int[][]numberOfMap, GameRule g) {
		...
		else if (g.canMoveTwoSteps(Control.LEFT, theTeam, numberOfRoles, numberOfMap, k, m) && j == m - 2 && i == k) {
			return Control.LEFT2;
		}
		else if (g.canMoveTwoSteps(Control.DOWN, theTeam, numberOfRoles, numberOfMap, k, m) && j == m + 2 && i == k) {
